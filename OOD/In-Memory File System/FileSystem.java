...
public class FileSystem {
	...
	public void delete(String path) {
		...
		String[] components = path.split("/");
		final String dirName = components[components.length - 1] == null ? components[components.length - 2]: components[components.length - 1];
		final Directory parent = (Directory) entries.get(entries.size() - 2); 
		parent.deleteEntry(parent.getChild(dirName));
	}
	public Entry[] list(String path) {
		...
		Directory target = (Directory) (entries.get(entries.size() - 1));
		return (Entry[]) target.getContents().toArray();
	}
